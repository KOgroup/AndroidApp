...
public class AngleMeter extends ApplicationAdapter {
	...
	public void create() {
		...
		//note window for saved results
		noteWindow();
		
		//set stage - most buttons are set here
		setStage();
		
		//adjusts which buttons should be visiblle atm
		onOff();
	...
	private void setFont() {
		...
		parameter.size = Math.round(TEXTSIZE * ratio-5);
	...
	private void setStage() {
		...
		//area where you can delete values
...
//		deleteArea.debug();
	...
	private void setButtonTable(float a) {
		...
		angle3.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				helpLabel.setText(
						"\u00b7Use this button to draw angles\nup to 30\u00B0 (e.g. on a piece of paper)\n\u00b7Bottom-left corner of the phone\nshould be fixed to avoid errors");
				state = ANGLE;
				state2 = AZIMUTH;
				onOff();
		...
		final ImageButton water = new ImageButton(skin, "b5");
	...
	//adjusts which buttons should be visible
	private void onOff() {
